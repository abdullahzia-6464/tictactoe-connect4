...
class Connect4QLearningAgent:
    ...
    def get_state_key(self, game):
        ...
        # For Connect4, we need to create a string representation of the board
        state = ''.join(''.join(row) for row in game.board) + game.current_player
        return state
    ...
    def get_move(self, game):
        ...
        if self.training and random.random() < self.epsilon:
            ...
            max_value = max(q_values.get(a, 0) for a in valid_moves)
            best_actions = [a for a in valid_moves if q_values.get(a, 0) == max_value]
    ...
    def learn(self, game, reward):
        ...
        old_q = self.q_table[self.last_state].get(self.last_action, 0)
...
def train_connect4_agent(episodes=5000, save_path="connect4_qtable.pkl"):
    """Train a Q-learning agent for Connect 4.
    
    Note: Due to the large state space of Connect 4, a reasonable training 
    requires fewer episodes but more focused exploration than Tic Tac Toe.
    """
    agent_x = Connect4QLearningAgent('X', epsilon=0.3)
    agent_o = Connect4QLearningAgent('O', epsilon=0.3)
    ...
    epsilon_decay = 0.999  # Slightly faster decay for Connect4
    min_epsilon = 0.05
    log_interval = 100  # More frequent logging for Connect4
...
if __name__ == "__main__":
    ...
    if train_new:
        episodes = int(input("Number of episodes (default: 5000): ") or 5000)
        agent = train_connect4_agent(episodes)
    else:
        # Load pre-trained agent
        agent = Connect4QLearningAgent('X', epsilon=0.05)
        agent.training = False
        if not agent.load_qtable("connect4_qtable.pkl"):
